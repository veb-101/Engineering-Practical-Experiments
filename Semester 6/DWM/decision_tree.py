...
print("Decision tree of depth 10 on Breast Cancer dataset..")

dec_clf = DecisionTree(max_depth=11)

scores = cross_val_score(dec_clf, X_train, y_train, cv=3, scoring='accuracy')


print(
    f"Cross validation scores: {round(scores[0], 2), round(scores[1], 2), round(scores[2], 2)}")

print(f"Mean Accuracy: {round(np.mean(scores), 2)}")
